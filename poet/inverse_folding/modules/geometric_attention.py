"""Geometric attention implementation from ESM3"""
# copied from https://github.com/evolutionaryscale/esm/blob/main/esm/layers/geom_attention.py
...
class GeometricReasoningOriginalImpl(nn.Module):
    def __init__(
        ...
        self.s_norm = nn.LayerNorm(c_s, elementwise_affine=bias)
        dim_proj = (
    ...
    def forward(self, s, affine, affine_mask, sequence_id=None, chain_id=None):
        if sequence_id is None:
            sequence_id = torch.zeros_like(s[..., 0], dtype=torch.int64)
        if chain_id is None:
            chain_id = torch.zeros_like(s[..., 0], dtype=torch.int64)
